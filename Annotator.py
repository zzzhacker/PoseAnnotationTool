...
from GUI import GUI
import hotkeys as keys
from PoseModel import pose_estimator
...
class Annotator(GUI):
    ...
    def __init__(
        ...
        if os.path.exists(datapath):
            self._new_init_data(datapath, dataset)
        else:
    ...
    def _save(self):
        ...
        """
        self.dataset["annotations"][self.image_idx] = self.skeleton.loc[
            :, ["x", "y"]
        ].values
        self.dataset["annotated"][self.image_idx] = self.skeleton.loc[
            :, "annotated"
        ].values
        self.skeleton.loc[:, ["x", "y"]] = self.dataset["annotations"][self.image_idx]
        self.skeleton.loc[:, "annotated"] = self.dataset["annotated"][self.image_idx]


    def _new_init_data(self, datapath, dataset):
        """ Initializes the images from the .h5 file (called in init).

        Parameters
        ----------
        datapath: str
            Path of directory where images are saved.
        ...
        if path.exists():
            self.dataset = {}
            self.dataset['data']= self._get_images(path,('*.png', '*.jpg', '*.jpeg'))
            self._resize()

        else:
            raise TypeError("Maybe directory is not prsest or dataset must be type `str`")

        self.n_images = len(self.dataset['data'])
        self.dataset['annotations'] = np.zeros((self.n_images, self.n_keypoints, 2),dtype=np.float64)
        self.dataset['annotated'] = np.zeros((self.n_images, self.n_keypoints), dtype=bool)

        

        if "skeleton" not in self.dataset:
            self.dataset['skeleton'] = self.skeleton[["tree", "swap_index"]].values

        # Unpack the images from the file
        self.image_idx = 0

        self.image = cv2.imread(str(self.dataset['data'][self.image_idx]))
        self._check_grayscale()
        self.skeleton.loc[:, ["x", "y"]] = pose_estimator.inference(self.image)
        self.skeleton.loc[:, "annotated"] = self.dataset["annotated"][self.image_idx]



    def _get_images(self,path,extensions):
        all_files = []
